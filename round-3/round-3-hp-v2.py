...
# --- Asymmetric trend protection (NEW) ---
# Submission 462864: peaked +12.7k at pos=+199, then drawdown -7.4k as mid drifted
# from 9997 to 9960 while we held the bag. Root cause: passive bids kept filling
# (bb+1 inside the market) while we were heavy long AND momentum was negative.
#
# Fix: when pos is heavy AND momentum is against us, suppress the SAME-side passive
# quotes so we stop accumulating into a losing position. Crucially, we DON'T
# suppress them when pos is light or when momentum is FOR us (preserves topside).
HP_MOM_WINDOW       = 50          # mid - mid_50_ago (in ticks of price)
HP_HEAVY_FRAC       = 0.50        # |pos|/limit above which gating activates
HP_MOM_THRESH       = 3.0         # |mom_50| above which gating activates
HP_FAST_EXIT        = True        # when heavy + adverse mom, post asks/bids deeper inside spread
...
class Trader:
    ...
    def _trade_hydrogel(self, state: TradingState, mem: dict) -> List[Order]:
        ...
        # ---- Momentum tracking for trend-aware gating ----
        hist = mem.setdefault("hp_mid_hist", [])
        hist.append(mid)
        if len(hist) > HP_MOM_WINDOW + 5:
            del hist[: len(hist) - HP_MOM_WINDOW - 5]
        mom = (mid - hist[-HP_MOM_WINDOW - 1]) if len(hist) > HP_MOM_WINDOW else 0.0
        ...
        # ---- 7. Asymmetric trend gating (NEW) ----
        # When pos is heavy in same direction as adverse momentum: KILL same-side quotes
        # to stop accumulating into a losing position. Preserves topside because we only
        # gate when BOTH conditions hold.
        pos_frac = pos / HP_LIMIT
        gate_buy = (pos_frac >= HP_HEAVY_FRAC and mom <= -HP_MOM_THRESH)
        gate_sell = (pos_frac <= -HP_HEAVY_FRAC and mom >= HP_MOM_THRESH)
        if gate_buy:
            buy_cap = 0       # heavy long + downtrend → STOP buying
        if gate_sell:
            sell_cap = 0      # heavy short + uptrend → STOP selling

        # ---- 8. Multi-tier passive quote layer ----
        ...
        # FAST EXIT (NEW): when heavy in adverse direction, post the EXIT side
        # deeper inside the spread for faster fills. This gives back some spread
        # but liquidates the losing position before mid drifts further.
        if HP_FAST_EXIT and pos_frac >= HP_HEAVY_FRAC and mom <= -HP_MOM_THRESH:
            # Heavy long + downtrend → post ask aggressively (deeper inside)
            my_ask = max(int(mid) + 1, fv_int + 2)
        elif HP_FAST_EXIT and pos_frac <= -HP_HEAVY_FRAC and mom >= HP_MOM_THRESH:
            my_bid = min(int(mid) - 1, fv_int - 2)

        # Tier 1 at the inside / Tier 2 at the market
