...
# ============================================================================
  ...
  [N] Take-profit FV boost (NEW — uses [M] infrastructure).
      When MTM > TAKE_PROFIT_THRESHOLD AND |position| ≥ TAKE_PROFIT_MIN_POS,
      bias FV toward neutral by TAKE_PROFIT_FV_BOOST ticks.  This loosens
      the buy clamp and makes the passive bid more aggressive (closer to mid),
      encouraging cover so we book gains and free position capacity for
      the next reversal cycle.
"""
import json
...
class Trader:
    ...
    LT_MEAN_BLEND = 0.30     # [L] τ ≈ 180 ticks (was 0.25 → τ=145).  Test 0.35, 0.40.
    LT_MEAN_ALPHA = 0.0005   # [L] slower (was 0.001) — keeps anchor at 9995 for the run
    ...
    # ----- Take-profit FV boost [N] (ON by default; tune as needed) ------
    TAKE_PROFIT_THRESHOLD = 2000  # MTM gain in XIRECS to start tightening
    TAKE_PROFIT_MIN_POS   = 100   # only apply when |position| ≥ this
    TAKE_PROFIT_FV_BOOST  = 5     # ticks of FV bias toward neutral
