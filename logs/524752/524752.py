...
class Trader:
    ...
    def _market_make(self, state: TradingState, mem: dict) -> List[Order]:
        ...
        # ----- Inventory skew (linear + nonlinear, time-decay scaled) -----
        # [Path E] Time-decay multiplier: ramps 1.0 → (1 + TIME_DECAY_BOOST)
        # over the run.  Encodes the A-S intuition that inventory carried near
        # the run boundary is much more dangerous than inventory carried early.
