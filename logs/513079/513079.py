...
class Trader:
    ...
    MIN_QUOTE_EDGE = 7    # [H] don't penny-improve closer than this to FV

    # ----- OU mean-reversion ---------------------------------------------
    LT_MEAN_PRIOR = 10010.0  # [I] shifted up from 9995 to cut short bias
    ...
    def _market_make(self, state: TradingState, mem: dict) -> List[Order]:
        ...
        # Microprice → FV briefly disagrees with mid during regime shifts;
        # crosses then fire at adverse prices and we get sniped.
        stale_fv = (self.STALE_FV_GUARD > 0
                    and abs(fv_int - mid) > self.STALE_FV_GUARD)

        # [K] Inventory soft cap: stop adding on the loaded side near limits.
        ...
        # [C/F] Pull BID when M14 just dumped large size OR M22 just bought
        skip_bid = (state.timestamp < mem.get('m14_high_sell_until', 0)
                    or state.timestamp < mem.get('m22_buy_until', 0))
        # [G]  Pull ASK when M14 just bought large size
        skip_ask = state.timestamp < mem.get('m14_high_buy_until', 0)
