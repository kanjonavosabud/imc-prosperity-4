...
class Trader:
    ...
    # IV smile fitted from R3 historical data: IV(m_t) = a*m_t^2 + b*m_t + c
    # m_t = log(S/K) / sqrt(T)
    SMILE_A = 0.142979
    SMILE_B = 0.002469
    SMILE_C = 0.235495

    # TTE in calendar days at the start of round 3 day 0.
    # Adjust if competition starts on a later day.
    TTE_INITIAL_DAYS = 8.0
    ...
    MM_PARAMS = {
        ...
        'HYDROGEL_PACK':       {'limit': 200, 'edge': 12, 'skew': 0.05, 'nl_skew': True, 'simple_micro': True},
        ...
        'VEV_5000':            {'limit': 300, 'edge': 2,  'skew': 0.005},
        'VEV_5100':            {'limit': 300, 'edge': 2,  'skew': 0.005},
        # Underlying
        'VELVETFRUIT_EXTRACT': {'limit': 200, 'edge': 4,  'skew': 0.005},
    }
    ...
    LT_MEAN_PRIOR = {'HYDROGEL_PACK': 9990.0, 'VELVETFRUIT_EXTRACT': 5250.0}
    ...
    # Imbalance signal: top-of-book 3:1 ratio. For HYDROGEL it's a genuine signal
    # (rare ~107 events / 30k ticks, 70-77% win rate, ±3 move). For VE/vouchers
    # this fires thousands of times with ~50% win rate — pure noise. So we only
    # use it on HYDROGEL.
    IMBALANCE_RATIO_THR = 3.0
    IMBALANCE_FV_SHIFT = {
        'HYDROGEL_PACK': 3.0,
    }
    ...
    def run(self, state: TradingState):
        ...
        for product, params in self.MM_PARAMS.items():
            if product in state.order_depths:
                result[product] = self.market_make(state, product, params, S, lt_means)
    ...
    def market_make(self, state: TradingState, product: str, params: dict,
                    underlying_mid, lt_means: dict) -> List[Order]:
        order_depth = state.order_depths[product]
        position = state.position.get(product, 0)
        orders: List[Order] = []
