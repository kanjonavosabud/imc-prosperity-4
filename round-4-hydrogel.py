...
class Trader:
    ...
    # [Path E] Time-decay on inventory aversion — A-S inspired.
    # Ramps from 1.0 at run start to (1 + TIME_DECAY_BOOST) at run end.
    # Captures the "can't carry inventory across boundary" intuition: as run
    # ends, every unit of |position| matters more because there's less time
    # for reversion to bail us out.
    # Effect: at pos=-180, end-of-run skew is ~3.6× current (was: 6 ticks of FV
    # shift; becomes ~21 ticks).  Bid clamps closer to mid → cheaper covers.
    # Aggressor BUY threshold rises → catches NPC asks during late drops.
    TIME_DECAY_BOOST    = 2.0     # FV shift multiplier at end of run vs start
    TIME_DECAY_TS_TOTAL = 1_000_000  # total ts in a run (1000 ticks × 100 ts/tick)

    # [Path F] OU optimal target-position framework — opt-in alternative to
    # the (linear+nonlinear) inventory skew + Path E time-decay.
    # When USE_PATH_F=True, the skew is replaced by:
    #     target_pos = -LIMIT × tanh((mid − lt_mean) / TARGET_SCALE)
    #     deviation  = position − target_pos
    #     fv_shift   = deviation × TARGETED_SKEW
    # Theoretical motivation: under stationary OU, the position that maximizes
    # expected P&L is monotonic in (mid − μ).  tanh keeps it bounded by LIMIT.
    # The skew now references DEVIATION from optimal, not raw position — so
    # at mid=μ target=0 (push toward neutral); at peak target=−LIMIT (push to
    # max short); at trough target=+LIMIT (push to max long).
    USE_PATH_F        = False     # opt-in toggle.  Set True to test.
    TARGET_SCALE      = 30.0      # ticks per σ in tanh denominator (~R4 σ̂ = 34.6)
    TARGETED_SKEW     = 0.05      # FV shift per unit deviation from target
    ...
    INNER_OFFSET_ASK = 1     # 522750: 0 lost 10 of 12 M38 SELL fills to NPC queue.
                              # Reverted to penny-improve.  -$705 vs baseline.
    ...
    PROBE_SIZE   = 0         # OFF: probes never fired in 522750 (deep, no flash events).
                              # Set 10 to test.
    PROBE_OFFSET = 5
    ...
    def _market_make(self, state: TradingState, mem: dict) -> List[Order]:
        ...
        # ----- Inventory skew -----
        if self.USE_PATH_F:
            # [Path F] Target-position framework.  fv_shift is proportional to
            # how far we are from the OU-optimal target_pos for the current mid.
            target_pos = self._target_position(mid, new_mean)
            deviation = position - target_pos
            fv_shift = deviation * self.TARGETED_SKEW
        else:
            # Default: linear+nonlinear inventory skew with [Path E] time-decay.
