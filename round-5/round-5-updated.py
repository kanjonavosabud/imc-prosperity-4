...
Rebuilt on top of the v1 (round-5.py) by:
  ...
  - replacing the simple MM with a MEAN-REVERSION MM (FV pulls back
    toward a slow EWMA) — first iteration used trend-continuation
    bias and lost $293k via 4.3x churn against a reverting tape,
...
# Mean-reversion FV. First version of this file used GAIN = +0.5
# (momentum continuation) and lost $293k on day-4 replay, with 4.3x
# the trade volume of krishi.py — every fill landed just before a
# reversal. The market is MEAN-REVERTING on tick-scale: short drifts
# revert before they extend. So the FV is pulled BACK toward the
# EWMA. fv = mid - 0.5*(mid - ewma) = mid + 0.5*(ewma - mid).
#   - mid below ewma → fv > mid → bid more aggressively (buy the dip)
#   - mid above ewma → fv < mid → ask more aggressively (sell the rip)
EWMA_ALPHA = 0.065
TREND_GAIN = -0.5                   # NEGATIVE = mean-reversion bias.
...
class Trader:
    ...
    def _mm_core(self, state: TradingState, prod: str, ewmas: dict,
        ...
        # Mean-reversion FV. With TREND_GAIN < 0, fv pulls TOWARD ewma:
        # if mid is currently below the recent average, fv sits above
        # mid → we bid the dip. The first version of this file ran
        # GAIN = +0.5 (continuation) and got run over because the
        # underlying tape mean-reverts on every-tick scale.
